...
async function ensureFolio(reservationId) {
    try {
        ...
        // A folio may not exist yet for a pending stay; create one below.
    }
    ...
    try {
        ...
        // Handle a concurrent request that created the folio first.
...
/**
 * Charge purchased services to the reservation folio linked to each room allocation.
 */
export const createServiceBookings = async (payload) => {
    ...
    if (payload.customerId) {
        ...
        active.forEach((allocation) => allocationMeta.set(Number(allocation.allocationId), allocation));
    }
